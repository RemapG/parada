...
const App = () => {
  ...
  const handleData = (data: any, conn: DataConnection) => {
    ...
    if (data.type === 'INVITE') {
      alert(`${data.inviter} invited you to join ${data.serverName}!`);
    }
  };
  ...
  const inviteFriendToServer = (friendPeerId: string) => {
    ...
    if (conn) {
      ...
      alert("Friend is not connected right now.");
    }
  };
  ...
  const toggleScreenShare = async () => {
    if (isScreenSharing) {
      ...
      try {
        ...
        // Auto stop UI when user clicks "Stop sharing" in browser native bar
    ...
    <div className="h-screen w-screen flex items-center justify-center bg-[#1e1f22]">
      <div className="bg-[#313338] p-10 rounded-lg shadow-2xl w-full max-w-[480px] border border-white/5">
        ...
        <div className="space-y-6">
          <div>
            <label className="block text-[12px] font-bold text-[#b5bac1] uppercase mb-2 tracking-wide">Username</label>
            <input 
              autoFocus 
          ...
          <button onClick={() => handleLogin((document.querySelector('input') as HTMLInputElement).value)} className="w-full bg-[#5865f2] py-3 rounded font-bold text-white hover:bg-[#4752c4] transition-all transform active:scale-[0.98] shadow-lg">Continue</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="flex h-screen w-screen bg-[#1e1f22] overflow-hidden">
      ...
      <aside className="w-60 bg-[#2b2d31] flex flex-col flex-shrink-0">
        {view === 'home' ? (
          <>
            <header className="h-12 border-b border-black/20 flex items-center px-4 shadow-sm bg-[#2b2d31]">
              <input className="w-full bg-[#1e1f22] text-xs p-1.5 rounded outline-none border border-black/20" placeholder="Find or start a conversation" />
            </header>
            <div className="flex-1 p-2 space-y-1 overflow-y-auto">
               <div className="flex items-center px-2 py-2 rounded bg-[#3f4147] text-white cursor-pointer transition-colors">
                  <UsersIcon size={20} className="mr-3 text-[#dbdee1]" /> <span className="font-medium">Friends</span>
               </div>
               <p className="text-[12px] font-bold text-[#949ba4] uppercase px-2 mt-4 mb-1 tracking-wider">Direct Messages</p>
               {friends.length === 0 ? (
                 ...
                 <div key={f.peerId} className="flex items-center px-2 py-1.5 rounded hover:bg-[#35373c] cursor-pointer group transition-colors">
                    <div className="relative">
                      <img src={f.avatar} className="w-8 h-8 rounded-full mr-3" />
          ...
          <>
            ...
            <div className="flex-1 p-2 overflow-y-auto">
               <div className="flex items-center justify-between text-[#949ba4] px-2 mb-1 group/header">
                  <span className="text-[12px] font-bold uppercase tracking-wider group-hover/header:text-[#dbdee1]">Channels</span>
               ...
               {activeServer?.channels.map(c => (
                 <div key={c.id} onClick={() => { setActiveChannelId(c.id); if(c.type === 'voice') joinVoiceChannel(); }} className={`flex items-center px-2 py-1.5 rounded mb-[2px] cursor-pointer group transition-all ${activeChannelId === c.id ? 'bg-[#3f4147] text-white shadow-sm' : 'text-[#949ba4] hover:bg-[#35373c] hover:text-[#dbdee1]'}`}>
        ...
        <div className="bg-[#232428] px-2 h-[52px] flex items-center gap-2 border-t border-black/10">
           <div className="relative cursor-pointer group">
              <img src={user.avatar} className="w-8 h-8 rounded-full border border-black/20" />
          ...
          <div className="flex-1 flex flex-col">
            <header className="h-12 border-b border-black/20 flex items-center px-4 gap-4 shadow-sm bg-[#313338]/95 backdrop-blur z-10">
              <div className="flex items-center text-[#dbdee1] font-bold border-r border-white/10 pr-4">
                <UsersIcon size={20} className="mr-2 opacity-60" /> Friends
              </div>
              <div className="flex gap-4 text-[#b5bac1] font-medium text-sm flex-1">
                <span className="text-white bg-[#3f4147] px-2 py-0.5 rounded cursor-pointer">Online</span>
                <span className="hover:bg-[#3f4147] hover:text-white px-2 py-0.5 rounded cursor-pointer transition-colors">All</span>
                <button onClick={() => setShowAddFriend(true)} className="bg-[#23a559] text-white px-3 py-0.5 rounded hover:bg-[#1a7a42] transition-colors ml-2 font-bold">Add Friend</button>
              </div>
            </header>
            <div className="flex-1 p-8 overflow-y-auto">
               {friends.length === 0 ? (
                 <div className="flex flex-col items-center justify-center h-full opacity-20 text-center">
                    ...
                    <p className="text-xl font-bold">Wumpus is waiting for friends...</p>
                    <p className="text-sm mt-2">Try adding someone using their unique ID!</p>
                 </div>
               ) : (
                 <div className="max-w-4xl mx-auto space-y-4">
                    <p className="text-[#949ba4] text-[12px] font-bold uppercase tracking-widest px-1">Online — {friends.length}</p>
                      ...
                      {friends.map(f => (
                        <div key={f.peerId} className="flex items-center justify-between p-3 border-t border-white/5 hover:bg-[#35373c] rounded-lg group transition-all cursor-pointer">
                           <div className="flex items-center gap-4">
                              <img src={f.avatar} className="w-10 h-10 rounded-full bg-[#1e1f22]" />
                              <div><p className="text-white font-bold">{f.username}</p><p className="text-xs text-[#b5bac1]">Online</p></div>
                           </div>
               ...
               {activeChannel?.type === 'text' ? (
                 <>
                    <div className="pb-10 pt-4 px-2">
                       ...
                       <p className="text-[#b5bac1] text-lg opacity-70">This is the start of the #{activeChannel?.name} channel.</p>
                    ...
                    <div className="space-y-1">
                      {(messages[activeChannelId || ''] || []).map(m => (
                        <div key={m.id} className="flex gap-4 group hover:bg-[#2e3035] -mx-4 px-4 py-1.5 transition-colors">
                           <img src={m.avatar} className="w-10 h-10 rounded-full flex-shrink-0 mt-0.5 cursor-pointer active:scale-95 transition-transform" />
                           <div className="flex-1 min-w-0">
                 ...
                 <div className="h-full w-full flex items-center justify-center flex-col gap-6 overflow-hidden">
                    ...
                    <div className="flex gap-6 bg-[#1e1f22]/80 backdrop-blur p-4 rounded-3xl shadow-2xl border border-white/10">
                       ...
                       <button 
                         ...
                         className={`p-4 rounded-full transition-all hover:scale-110 shadow-lg ${isScreenSharing ? 'bg-[#23a559]' : 'bg-[#4e5058] hover:bg-[#6d6f78]'}`}
                        >
              ...
              <div className="px-4 pb-6 mt-auto">
                <div className="bg-[#383a40] rounded-lg px-4 py-2.5 flex items-center gap-4 shadow-inner">
                   ...
                   <form onSubmit={(e) => { e.preventDefault(); sendMessage((e.target as any).msg.value); (e.target as any).reset(); }} className="flex-1">
                     <input name="msg" autoComplete="off" className="bg-transparent w-full outline-none text-[#dbdee1] placeholder-[#6d6f78] text-[15px]" placeholder={`Message #${activeChannel?.name}`} />
                   </form>
      ...
      {showCreateServer && (
        <div className="modal-backdrop" onClick={() => setShowCreateServer(false)}>
          <div className="bg-[#313338] p-8 rounded-lg w-full max-w-[440px] shadow-2xl" onClick={e => e.stopPropagation()}>
            <h2 className="text-2xl font-bold text-white text-center mb-2">Create your server</h2>
            <p className="text-[#b5bac1] text-center mb-8">Give your new server a personality with a name.</p>
            <input id="sname" className="w-full bg-[#1e1f22] text-white p-3 rounded mb-8 outline-none border border-black/20 focus:border-[#5865f2] transition-colors" placeholder="Server Name" defaultValue={`${user.username}'s server`} />
            <div className="flex justify-between items-center bg-[#2b2d31] -mx-8 -mb-8 p-4 rounded-b-lg mt-4">
               <button onClick={() => setShowCreateServer(false)} className="text-white hover:underline px-4">Cancel</button>
               <button onClick={() => createServer((document.getElementById('sname') as HTMLInputElement).value)} className="bg-[#5865f2] text-white px-8 py-2.5 rounded font-bold hover:bg-[#4752c4] transition-all">Create</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-[#313338] p-8 rounded-lg w-full max-w-[440px] shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4 mb-8">
               <div onClick={() => setNewChannelType('text')} className={`p-4 rounded-lg cursor-pointer border flex justify-between items-center ${newChannelType === 'text' ? 'bg-[#3f4147] border-[#5865f2]' : 'bg-[#2b2d31] border-transparent hover:bg-[#35373c]'}`}>
                  <div className="flex items-center gap-4"><Hash size={24} className="text-[#949ba4]" /><div className="text-left"><p className="text-white font-bold">Text</p><p className="text-xs text-[#949ba4]">Send messages, GIFs, and emojis</p></div></div>
               ...
               <div onClick={() => setNewChannelType('voice')} className={`p-4 rounded-lg cursor-pointer border flex justify-between items-center ${newChannelType === 'voice' ? 'bg-[#3f4147] border-[#5865f2]' : 'bg-[#2b2d31] border-transparent hover:bg-[#35373c]'}`}>
                  <div className="flex items-center gap-4"><Volume2 size={24} className="text-[#949ba4]" /><div className="text-left"><p className="text-white font-bold">Voice</p><p className="text-xs text-[#949ba4]">Hang out together with voice and video</p></div></div>
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${newChannelType === 'voice' ? 'border-[#5865f2]' : 'border(--discord-blurple)'}`}>{newChannelType === 'voice' && <div className="w-2.5 h-2.5 rounded-full bg-[#5865f2]"></div>}</div>
               </div>
            </div>

            <label className="text-[12px] font-bold text-[#b5bac1] uppercase mb-2 block tracking-wider">Channel Name</label>
            <input id="cname" className="w-full bg-[#1e1f22] text-white p-3 rounded mb-10 outline-none border border-black/20 focus:border-[#5865f2]" placeholder="new-channel" />

            <div className="flex justify-end items-center gap-4 bg-[#2b2d31] -mx-8 -mb-8 p-4 rounded-b-lg">
               <button onClick={() => setShowCreateChannel(false)} className="text-white hover:underline text-sm font-medium">Cancel</button>
               <button onClick={() => createChannel((document.getElementById('cname') as HTMLInputElement).value, newChannelType)} className="bg-[#5865f2] text-white px-8 py-2.5 rounded font-bold hover:bg-[#4752c4] transition-all">Create Channel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showAddFriend && (
        <div className="modal-backdrop" onClick={() => setShowAddFriend(false)}>
          <div className="bg-[#313338] p-8 rounded-lg w-full max-w-[440px] shadow-2xl" onClick={e => e.stopPropagation()}>
            <h2 className="text-xl font-bold text-white mb-2 uppercase tracking-wide">Add Friend</h2>
            <p className="text-[#b5bac1] text-sm mb-6 leading-relaxed">Enter your friend's unique VibeSpace ID:</p>
            ...
            <div className="flex justify-end gap-2 bg-[#2b2d31] -mx-8 -mb-8 p-4 rounded-b-lg">
               <button onClick={() => setShowAddFriend(false)} className="text-white hover:underline px-6 py-2 text-sm font-medium">Cancel</button>
               <button onClick={() => addFriend((document.getElementById('fid') as HTMLInputElement).value)} className="bg-[#5865f2] text-white px-8 py-2.5 rounded font-bold hover:bg-[#4752c4] transition-all">Send Friend Request</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showInviteModal && (
        <div className="modal-backdrop" onClick={() => setShowInviteModal(false)}>
          <div className="bg-[#313338] p-8 rounded-lg w-full max-w-[480px] shadow-2xl" onClick={e => e.stopPropagation()}>
             <div className="flex justify-between items-center mb-6">
                <h2 className="text-white font-bold flex items-center gap-3 text-xl"><UserPlus size={24} className="text-[#23a559]" /> Invite friends to {activeServer?.name}</h2>
                <X className="text-[#949ba4] cursor-pointer hover:text-white" onClick={() => setShowInviteModal(false)} />
             </div>
             
             <div className="space-y-4 mb-8 max-h-60 overflow-y-auto pr-2">
                <p className="text-[#b5bac1] text-[12px] font-bold uppercase tracking-widest px-1">Friend List</p>
                {friends.length === 0 ? (
                  <div className="p-4 bg-black/20 rounded-lg text-center"><p className="text-xs text-[#949ba4]">You have no friends to invite yet!</p></div>
                ) : friends.map(f => (
                  <div key={f.peerId} className="flex items-center justify-between p-2 rounded-lg hover:bg-[#2b2d31] transition-colors">
                     ...
                     <button onClick={() => inviteFriendToServer(f.peerId)} className="border border-[#5865f2] text-[#5865f2] hover:bg-[#5865f2] hover:text-white px-3 py-1 rounded text-xs font-bold transition-all active:scale-95">Invite</button>
                  </div>
                ))}
             </div>
             <div className="flex gap-2 bg-[#1e1f22] p-2 rounded border border-black/40 shadow-inner">
                <input readOnly value={`VibeSpace://${user?.id}`} className="bg-transparent flex-1 text-sm text-[#dbdee1] outline-none px-2 font-mono" />
                <button onClick={() => { navigator.clipboard.writeText(user?.id || ''); setCopied(true); setTimeout(()=>setCopied(false), 2000); }} className={`px-5 py-2 rounded text-sm font-bold transition-all ${copied ? 'bg-[#23a559]' : 'bg-[#5865f2] hover:bg-[#4752c4]'} text-white`}>{copied ? 'Copied!' : 'Copy'}</button>
             </div>
          </div>
        </div>
      )}
